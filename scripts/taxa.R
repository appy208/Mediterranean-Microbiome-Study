...
# Write it out as a TSV file
write.table(meta_df, "Final_Metadata_With_Intervention_Status.tsv",
...
levels(as.factor(metadata$`MedA...Post`))
levels(as.factor(metadata$`X2..MedDt.A`))
levels(as.factor(metadata$`Obese`))
...
#Clean up metadata, slightly
metadata$MedA_Post_ord <- factor(metadata$`MedA...Post`, 
                                 levels = c("Baseline", "MedA - Post", "MedWL - Post"))
levels(metadata$MedA_Post_ord)

metadata$MedDtA_ord <- factor(metadata$`X2..MedDt.A`, 
                              levels = c("1. MedDt-WL", "2. MedDt-A"))
levels(metadata$MedDtA_ord)

metadata$Obese_ord <- factor(metadata$Obese, levels = c("Overweight", "Obese"))
levels(metadata$Obese_ord)
...
my_column <- "MedA...Post"  #this is the metadata column that we will use in the taxa barplot
my_column_ordered <- c("Baseline", "MedA - Post", "MedWL - Post")
...
my_column2 <- "X2..MedDt.A"  #this is the metadata column that we will use in the taxa barplot
my_column_ordered2 <- c("1. MedDt-WL", "2. MedDt-A")
...
for(ml in my_level){
  ...
  taxa.summary <- physeq_bar_plot %>%
    ...
    transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance
    psmelt()  %>%                               # Melt to long format
    group_by(get(my_column2), get(ml)) %>%
    summarise(Abundance.average=mean(Abundance)) 
  taxa.summary <- as.data.frame(taxa.summary)
  colnames(taxa.summary)[1] <- my_column2
  colnames(taxa.summary)[2] <- ml
  ...
  physeq_meta_filtered$my_column_ordered2 = factor(physeq_meta_filtered[[my_column2]], my_column_ordered2)
  ...
  # Plot 
  ggplot(physeq_meta_filtered, aes(x = my_column_ordered2, y = Abundance.average, fill = get(ml))) + 
    #facet_grid(.~.) +
    geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = my_colors) +
    # Remove x axis title
    ...
    xlab(my_column2) +
    ggtitle(paste0(ml, " (>", abund_filter * 100,"%) in at least 1 sample")) 
  ggsave(paste0("output/taxa/", ml, "BarPlot_", my_column2, ".png"), height = 5, width = 4)
}


my_column3 <- "Obese"  #this is the metadata column that we will use in the taxa barplot
my_column_ordered3 <- c("Overweight", "Obese")
...
for(ml in my_level){
  ...
  taxa.summary <- physeq_bar_plot %>%
    ...
    transform_sample_counts(function(x) {x/sum(x)} ) %>% # Transform to rel. abundance
    psmelt()  %>%                               # Melt to long format
    group_by(get(my_column3), get(ml)) %>%
    summarise(Abundance.average=mean(Abundance)) 
  taxa.summary <- as.data.frame(taxa.summary)
  colnames(taxa.summary)[1] <- my_column3
  colnames(taxa.summary)[2] <- ml
  ...
  physeq_meta_filtered$my_column_ordered3 = factor(physeq_meta_filtered[[my_column3]], my_column_ordered3)
  ...
  # Plot 
  ggplot(physeq_meta_filtered, aes(x = my_column_ordered3, y = Abundance.average, fill = get(ml))) + 
    #facet_grid(.~.) +
    geom_bar(stat = "identity", position = position_stack(reverse = TRUE)) +
    scale_fill_manual(values = my_colors) +
    # Remove x axis title
    ...
    xlab(my_column3) +
    ggtitle(paste0(ml, " (>", abund_filter * 100,"%) in at least 1 sample")) 
  ggsave(paste0("output/taxa/", ml, "BarPlot_", my_column3, ".png"), height = 5, width = 4)
}
